...
function pickKeyValue(record: Record<string, unknown>): string | undefined {
...
export function extractApiKeys(payload: unknown): ResolvedApiKeys {
  const out: ResolvedApiKeys = {};
  ...
  if (typeof record.anon_key === "string") out.anonKey = record.anon_key;
  if (typeof record.service_role_key === "string") out.serviceRoleKey = record.service_role_key;

  const collections: unknown[] = [];
  if (Array.isArray(payload)) collections.push(payload);
  if (Array.isArray(record.keys)) collections.push(record.keys);
  if (Array.isArray(record.data)) collections.push(record.data);
...
async function hydrateKeysFromManagement(context: RequestContext): Promise<ResolvedApiKeys> {
...
export async function requireServiceRoleClient(context: RequestContext): Promise<SupabaseClient> {
  if (context.supabase.serviceRoleClient) {
  ...
  throw new AppError(
    ...
    "Service-role key unavailable. Provide SUPABASE_SERVICE_ROLE_KEY or allow get_api_keys via Management API.",
    400,
  );
}

export async function resolveApiKeys(context: RequestContext): Promise<ResolvedApiKeys> {
