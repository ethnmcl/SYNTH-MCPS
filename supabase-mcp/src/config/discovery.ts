...
export interface DiscoveryResult {
  ...
  checks: {
    projectUrl: { reachable: boolean; status?: number; error?: string };
    restUrl: { reachable: boolean; status?: number; error?: string };
...
export async function discoverSupabaseConfig(input: DiscoveryInput): Promise<DiscoveryResult> {
  if (!input.projectRef) {
  ...
  if (input.accessToken) {
    try {
      const project = await fetchJson<ProjectMetadata>(
        `${urls.managementApiUrl}/projects/${input.projectRef}`,
        {
          headers: {
...
export function toGeneratedEnv(discovery: DiscoveryResult): string {
  const lines = [
