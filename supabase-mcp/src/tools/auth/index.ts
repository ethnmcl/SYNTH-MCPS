...
export const authTools: Record<string, ToolHandler> = {
  ...
  async create_user(context, input) {
    ...
    const { data, error } = await client.auth.admin.createUser({
      ...
      user_metadata: (input.user_metadata as Record<string, unknown>) ?? undefined,
    });
    if (error) throw error;
  ...
  async update_user(context, input) {
    ...
    const { error } = await client.auth.admin.updateUserById(userId, updates as any);
    if (error) throw error;
    return { success: true };
  },
