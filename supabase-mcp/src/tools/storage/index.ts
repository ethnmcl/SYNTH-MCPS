...
function parseUploadContent(content: string, encoding: "text" | "base64"): Buffer {
  if (encoding === "base64") return Buffer.from(content, "base64");
  return Buffer.from(content, "utf8");
}

export const storageTools: Record<string, ToolHandler> = {
  ...
  async upload_file(context, input) {
    ...
    const encoding = (input.encoding === "base64" ? "base64" : "text") as "base64" | "text";
