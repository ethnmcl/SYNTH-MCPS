...
const ListProjectsOut = z.object({ projects: z.array(z.record(JsonValueSchema)) });
const CreateProjectIn = z
  .object({
    name: z.string().min(1),
  ...
  .merge(ConfirmDangerousSchema);
const PauseProjectIn = z.object({ project_ref: z.string().min(1).optional() }).merge(ConfirmDangerousSchema);
const ResumeProjectIn = z.object({ project_ref: z.string().min(1).optional() }).merge(ConfirmDangerousSchema);
const DeleteProjectIn = z.object({ project_ref: z.string().min(1).optional() }).merge(ConfirmDangerousSchema);
...
export const TOOL_DEFINITIONS: ToolDefinition[] = [
  ...
  { name: "list_projects", description: "List projects available to the current access token.", group: "project_admin", access_level: "admin", dangerous: false, confirmation_required: false, audit_log: true, inputSchema: z.object({}), outputSchema: ListProjectsOut },
  { name: "create_project", description: "Create a new Supabase project.", group: "project_admin", access_level: "admin", dangerous: true, confirmation_required: true, audit_log: true, inputSchema: CreateProjectIn, outputSchema: z.object({ success: z.boolean(), project_ref: z.string().optional(), project_id: z.string().optional(), status: z.string().optional() }) },
  { name: "pause_project", description: "Pause an existing Supabase project.", group: "project_admin", access_level: "admin", dangerous: true, confirmation_required: true, audit_log: true, inputSchema: PauseProjectIn, outputSchema: z.object({ success: z.boolean(), project_ref: z.string(), status: z.string().optional() }) },
  { name: "resume_project", description: "Resume a paused Supabase project.", group: "project_admin", access_level: "admin", dangerous: true, confirmation_required: true, audit_log: true, inputSchema: ResumeProjectIn, outputSchema: z.object({ success: z.boolean(), project_ref: z.string(), status: z.string().optional() }) },
  { name: "delete_project", description: "Delete a Supabase project.", group: "project_admin", access_level: "admin", dangerous: true, confirmation_required: true, audit_log: true, inputSchema: DeleteProjectIn, outputSchema: z.object({ success: z.boolean(), project_ref: z.string() }) },
];
