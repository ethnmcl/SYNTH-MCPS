...
function parseArgs(): { mode: "list" } | { mode: "call"; tool: string; input: Record<string, unknown> } {
  const [, , mode, arg1, arg2] = process.argv;
  if (mode === "list") return { mode: "list" };
  if (mode === "call") {
    if (!arg1) usage();
    const input = arg2 ? (JSON.parse(arg2) as Record<string, unknown>) : {};
...
async function main(): Promise<void> {
  ...
  const resolvedServerCommand = process.env.MCP_SERVER_COMMAND ?? "node";
  ...
  const client = new Client(
    { name: "supabase-mcp-cli", version: "1.0.0" },
    { capabilities: {} },
  );
