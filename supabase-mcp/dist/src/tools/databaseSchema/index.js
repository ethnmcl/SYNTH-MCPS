...
export const databaseSchemaTools = {
    ...
    async alter_table(context, input) {
        ...
        for (const op of operations) {
            ...
            if (action === "add_column") {
                await context.managementApi.executeSql(`alter table ${qualifyTable(schema, table)} add column if not exists ${quoteIdent(String(op.column))} ${String(op.column_type)};`);
            }
            else if (action === "drop_column") {
                await context.managementApi.executeSql(`alter table ${qualifyTable(schema, table)} drop column if exists ${quoteIdent(String(op.column))};`);
            }
            else if (action === "rename_column") {
                await context.managementApi.executeSql(`alter table ${qualifyTable(schema, table)} rename column ${quoteIdent(String(op.column))} to ${quoteIdent(String(op.new_name))};`);
            }
            else if (action === "alter_column_type") {
                await context.managementApi.executeSql(`alter table ${qualifyTable(schema, table)} alter column ${quoteIdent(String(op.column))} type ${String(op.column_type)};`);
            }
        }
    ...
    async create_index(context, input) {
        const schema = String(input.schema ?? "public");
        const table = String(input.table);
        const columns = input.columns.map((col) => quoteIdent(col)).join(", ");
        const unique = input.unique === true ? "unique " : "";
        const derivedName = `${table}_${input.columns.join("_")}_idx`;
        const indexName = String(input.index_name ?? derivedName);
        await context.managementApi.executeSql(`create ${unique}index if not exists ${quoteIdent(indexName)} on ${qualifyTable(schema, table)} (${columns});`);
        return { success: true, index_name: indexName };
    },
    async list_migrations(context) {
        await ensureMigrationTable(context);
        const rows = await context.managementApi.executeSql("select id, name, applied_at from public.mcp_migrations order by applied_at desc;");
    ...
    async apply_migration(context, input) {
        ...
        try {
            await context.managementApi.executeSql(sql);
            await context.managementApi.executeSql(`insert into public.mcp_migrations (id, name, sql) values ('${id}', '${name.replace(/'/g, "''")}', '${sql.replace(/'/g, "''")}');`);
