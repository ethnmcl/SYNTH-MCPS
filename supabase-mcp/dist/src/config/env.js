...
import { z } from "zod";
import { AccessLevelSchema, ToolGroupSchema } from "../schemas/permissions.js";
import { deriveSupabaseUrls } from "./urls.js";
dotenv.config();
const EmptyToUndefinedStringSchema = z.preprocess((value) => (typeof value === "string" && value.trim() === "" ? undefined : value), z.string().optional());
const OptionalUrlSchema = z.preprocess((value) => (typeof value === "string" && value.trim() === "" ? undefined : value), z.string().url().optional());
...
export function getEnv(options = {}) {
    ...
    if (!parsed.SUPABASE_PROJECT_REF && !allowPartial) {
        throw new Error("SUPABASE_PROJECT_REF is required. Set it directly or run `npm run discover:supabase` after exporting SUPABASE_ACCESS_TOKEN.");
    }
...
export function requiredSecretPresence(env) {
    return {
