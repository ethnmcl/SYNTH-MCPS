...
export const closeIssueTool: GithubTool<typeof schema> = {
  ...
  execute: async (ctx, input) => {
    ...
    const { data } = await ctx.octokit.rest.issues.update({ ...input, state: 'closed' });
    return result(`Closed issue #${data.number}`, { number: data.number, state: data.state, html_url: data.html_url });
  }
};
